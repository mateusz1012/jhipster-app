...
describe('Entry e2e test', () => {
  ...
  it('should load Entries', async () => {
    ...
    expect(await entryComponentsPage.getTitle()).to.eq('mySecondApp.entry.home.title');
  });

  it('should load create Entry page', async () => {
    await entryComponentsPage.clickOnCreateButton();
    entryUpdatePage = new EntryUpdatePage();
    expect(await entryUpdatePage.getPageTitle()).to.eq('mySecondApp.entry.home.createOrEditLabel');
  ...
  it('should create and save Entries', async () => {
    ...
    await entryComponentsPage.clickOnCreateButton();
    await promise.all([
      entryUpdatePage.setFilmTitleInput('filmTitle'),
      entryUpdatePage.setDescriptionInput('description'),
      entryUpdatePage.setDateInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
      entryUpdatePage.filmSelectLastOption()
      // entryUpdatePage.tagSelectLastOption(),
    ]);
    expect(await entryUpdatePage.getFilmTitleInput()).to.eq('filmTitle', 'Expected FilmTitle value to be equals to filmTitle');
    expect(await entryUpdatePage.getDescriptionInput()).to.eq('description', 'Expected Description value to be equals to description');
  ...
  it('should delete last Entry', async () => {
    ...
    expect(await entryDeleteDialog.getDialogTitle()).to.eq('mySecondApp.entry.delete.question');

...
export class EntryUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.filmService
      .query()
      .subscribe((res: HttpResponse<IFilm[]>) => (this.films = res.body), (res: HttpErrorResponse) => this.onError(res.message));
    this.tagService
      .query()
  ...
  private createFromForm(): IEntry {
    return {
      ...
      filmTitle: this.editForm.get(['filmTitle']).value,
  ...
  trackFilmById(index: number, item: IFilm) {
    return item.id;
  }
